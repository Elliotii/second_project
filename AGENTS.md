...
This repository is building an Agent Harness Reliability Workbench. V0-A, V0-B and V0-C are closed and accepted, so the minimal usable V0 Workbench is complete. V1-A is closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; its independently re-audited Implementation Baseline is `784bd1ec06c2aa9ed554a7da661bdf582097bcdf`. V1-B is historically closed as `CLOSE_V1_B_INCONCLUSIVE_AUTHORIZED_SEQUENCE_EXHAUSTED`, not completed, with zero valid comparable Runs. V1-C is the active Goal under the accepted `V1_C_GOAL_CONTRACT.md`. Its zero-real-call Stage 1 corrected Candidate `962b42a281d3092f0faf399b9f6f1ecaa0212f31` passed fresh focused re-audit and is accepted as `PASS_V1_C_STAGE1_AUDITED_CANDIDATE`. The disjoint real Canary completed terminal, integrity-valid and comparable with `task_pass`, eight Provider/model requests and exact tracked cost USD `0.00042865199999999996`; Main accepts it as `PASS_V1_C_REAL_CANARY`. The first fresh full-Pilot Session then stopped before Workbench initialization with zero Pilot cells, calls and cost because its ignored opaque preload semantics drifted from the accepted Canary helper. Main accepts that fail-closed pause. The user authorizes one and only one fresh preinitialization continuation from a new Main-owned baseline, using the exact accepted Canary helper semantics and SHA-256 `2d83b0e1e3eafecb774a3e6faf4f6ac1ec29984ee256dc750a06805e3f577438`. The immutable 24-cell Manifest, maximum-eight-child and USD 1.90 Pilot boundary remain unchanged. Automatic Pause Conditions remain binding; a second preinitialization failure closes V1-C inconclusive. V2, source repair during real execution, retry, fallback and replacement remain unauthorized. V1-B, Canary and first pause evidence stay immutable and the Canary is not a Pilot member.
...
- V1-C full Pilot authority is frozen to `fixtures/manifests/v1/v1c-full-pilot-execution.json`, 24 initial cells, at most 8 child Attempts and USD 1.90. The first full-Pilot wrapper stopped before initialization with zero product state or calls; its authority is consumed. Exactly one fresh continuation may use an ignored loader whose bytes and behavior match the accepted Canary helper SHA-256 `2d83b0e1e3eafecb774a3e6faf4f6ac1ec29984ee256dc750a06805e3f577438`; Main and the execution Session must not inspect, normalize, edit or log the Credential file/value. Execute exactly one next cell per tracked `run-next` process. The fresh execution Session may read the opaque DeepSeek Credential and use the fixed endpoint only inside each bounded process; it may not edit source, tests, fixtures, Manifest, Prompt, Skill, task, Verifier, Tool profile, Provider/model or control state. Stop at the first unknown usage/cost, evidence conflict, invalid-threshold stop, repeated invalid cause, nonterminal earlier cell, secret boundary failure or other Contract Pause Condition. No retry, fallback, replacement or V2. If the fresh continuation fails again before initialization, stop and close V1-C inconclusive without another normalization or execution loop.
