...
This repository is building an Agent Harness Reliability Workbench. V0-A, V0-B and V0-C are closed and accepted, so the minimal usable V0 Workbench is complete. V1-A is also closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; its independently re-audited Implementation Baseline is `784bd1ec06c2aa9ed554a7da661bdf582097bcdf`. There is currently no active Goal. The user has accepted `docs/reports/V1_B_INTEGRATED_DEVELOPMENT_PLAN.md` and the corresponding bounded V1 Charter clarification: V1-B remains one Goal, with a zero-real-call Stage 1 Preparation Session, a focused independent Candidate audit, and a fresh no-source-edit Stage 2 Execution Session. The V1-B Goal Contract, Activation, Stage 1, credentials, network access, real-model calls and Pilot budget are not yet authorized. V1 compares Baseline, Skill-only and Skill + External Verifier / Runtime Control without presuming the winner; V2 bounded multi-path recovery remains the Portfolio North Star, not current implementation scope. Do not reopen or silently modify V1-A while planning V1-B.
