...
This repository is building an Agent Harness Reliability Workbench. V0-A, V0-B and V0-C are closed and accepted, so the minimal usable V0 Workbench is complete. V1-A is closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; its independently re-audited Implementation Baseline is `784bd1ec06c2aa9ed554a7da661bdf582097bcdf`. V1-B is the active Goal under the accepted `V1_B_GOAL_CONTRACT.md`. Stage 1 corrected Candidate `a11690e5827d9d540b731156799566bea21c689e` passed focused independent re-audit, and Execution Baseline `19617319c13a9eecbb325682c920e79d1517b89d` passed zero-call Gates K/L. The first real Stage 2 cell then started once and paused with sanitized `FixedProviderBoundaryErrorV1B`; no later cell started and actual usage/cost remain unknown because the frozen pause path did not persist counter/reservation evidence. V1-B is therefore active and paused, not closed. The user has accepted `V1_B_PAUSE_RECOVERY_AMENDMENT.md`, authorizing one zero-call correction by the original Preparation Session, focused re-audit, a new Execution Baseline and one fresh replacement Pilot with USD 0.10 conservative prior debit, USD 1.90 remaining hard cap and at most 25 started initial Runs across both Pilots. Historical evidence is immutable; no same-Run retry, fallback or automatic replacement is allowed. V2 remains the Portfolio North Star and is not authorized.
