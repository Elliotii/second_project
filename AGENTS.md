...
> Goal 2 is activated. Real access is authorized only after the zero-access implementation,
> reference calibration, bounded implementation commit and Main preflight pass.
...
- V3.5 is governed by accepted `V3_5_CHARTER.md`. Goal 1 is closed and accepted while preserving `Session != Run`: public Pi JSONL is resumable conversation/context authority; Workbench Run artifacts remain execution/evaluation authority; the single-writer catalog is navigation metadata only. Full raw Session remains local Runtime state while CLI/API/UI/report consumers receive only allowlisted safe projections. The accepted claim is settled cross-process reopen/continue with deterministic/Faux evidence, not crash recovery, exactly-once Tool effects, database durability or external-model behavior. Automatic catalog rebuild remains a recorded productization limitation; missing/corrupt catalog currently fails closed. Goal 2 is active under `V3_5_G2_REAL_ADAPTIVE_SKILL_CASE_CONTRACT.md`. One new top-level Goal 2 Session owns the zero-access implementation and, only after a frozen implementation baseline and Main preflight, one Base then one Candidate real arm under the USD 0.40 pair cap. Once the first real Provider request occurs, source/fixture/Verifier/authority/budgets are immutable; retry, fallback, replacement, extra Case/arm, Pi patch/private import and SDK/Extension/RPC/server switching are forbidden. Goal 3 and final V3.5 acceptance remain unauthorized.
