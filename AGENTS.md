...
This repository is building an Agent Harness Reliability Workbench. V0-A, V0-B and V0-C are closed and accepted, so the minimal usable V0 Workbench is complete. V1-A is closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; its independently re-audited Implementation Baseline is `784bd1ec06c2aa9ed554a7da661bdf582097bcdf`. V1-B is historically closed as `CLOSE_V1_B_INCONCLUSIVE_AUTHORIZED_SEQUENCE_EXHAUSTED`, not completed, with zero valid comparable Runs. V1-C is the active Goal under the accepted `V1_C_GOAL_CONTRACT.md`. Its zero-real-call Stage 1 corrected Candidate `962b42a281d3092f0faf399b9f6f1ecaa0212f31` passed fresh focused re-audit as `PASS_FOCUSED_REAUDIT` and is accepted as `PASS_V1_C_STAGE1_AUDITED_CANDIDATE`. The resulting Main-owned revision freezes the audited Execution Baseline, one-cell Canary Manifest and zero-call Provider checkpoint without changing audited Workbench source/tests. Credential access, real network/Provider/model calls, Canary execution, the full A/B/C Pilot and V2 remain unauthorized. V1-B evidence remains immutable; V1-C uses new identities and cannot retroactively terminalize V1-B.
