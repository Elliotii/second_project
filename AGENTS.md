...
This repository is building an Agent Harness Reliability Workbench. V0-A, V0-B and V0-C are closed and accepted, so the minimal usable V0 Workbench is complete. V1-A is closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; its independently re-audited Implementation Baseline is `784bd1ec06c2aa9ed554a7da661bdf582097bcdf`. V1-B is the active Goal under the accepted `V1_B_GOAL_CONTRACT.md`. Its first Stage 1 Session `019fcc05-43a5-7481-8cb7-7d51e2d7085f` stopped before implementation with zero source delta after detecting stale inactive prose in `CURRENT_STATE.md`; the Main Session is correcting only that control narrative and will start a fresh dedicated Session from the corrected Control Baseline. Stage 1 permits only the Contract-listed bounded source/test/report work with zero credential reads, zero network, zero external-provider/model calls, no dependency installation, no Pi changes/private imports, no control-state edits and no dedicated-session Git commit. Candidate Commit, focused audit, Execution Baseline and real Stage 2 remain separate unauthorized control points. V1 compares Baseline, Skill-only and Skill + External Verifier / Runtime Control without presuming the winner; V2 bounded multi-path recovery remains the Portfolio North Star, not current implementation scope.
