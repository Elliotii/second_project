...
This repository is building an Agent Harness Reliability Workbench. V0-A, V0-B and V0-C are closed and accepted, so the minimal usable V0 Workbench is complete. V1-A is closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; V1-B is historically closed inconclusive with zero valid comparable Runs. V1-C remains the active Goal and exists to complete the frozen Baseline / Skill-only / Skill + Runtime Control comparison. Its accepted real Canary remains valid. A first full-Pilot continuation produced four terminal, integrity-valid and comparable Runs, including one real C-arm bounded Recovery, before a read-only aggregate-normalizer defect was found. Those four Runs remain immutable diagnostic evidence but are excluded from the formal restarted comparison. Candidate `5b87b98e431663595e9bd26a54589defbabcd3b1`, tree `a238d838c4197aae1ef35dd70507b680160e0c65`, corrects only the aggregate fairness analyzer and passed focused independent re-audit with no P1/P2/P3 finding. The disjoint R2 Pilot then produced four terminal, integrity-valid and comparable Runs; cell 04's product evidence closed successfully, but an incorrectly short outer command-runner wait returned `124` and lost the product-process exit code. Main accepted this as an orchestration observation defect, preserved cell 04 in the denominator without retry, and authorized only continuation from unstarted cell 05 with a safe outer wait. V2 remains unauthorized.
...
- V1-C's restarted full Pilot is frozen to `fixtures/manifests/v1/v1c-full-pilot-execution-r2.json`, 24 initial cells, at most 8 child Attempts and USD 1.90. Use the disjoint `.runs/v1-c/full-pilot-r2/pilot` root and never rewrite or merge the historical four-Run prefix. The execution Session must use the accepted opaque loader semantics; Main and the Session must not inspect, normalize, edit or log the Credential file/value. Execute exactly one next cell per tracked `run-next` process. The fresh execution Session may read the opaque DeepSeek Credential and use the fixed endpoint only inside each bounded process; it may not edit source, tests, fixtures, Manifest, Prompt, Skill, task, Verifier, Tool profile, Provider/model or control state. Pre-product mechanical command/path mistakes with zero Credential/Pilot/network/model side effects may be corrected and do not consume a Run; stop at the first actual unknown usage/cost, evidence conflict, invalid-threshold stop, repeated invalid cause, nonterminal earlier cell, secret boundary failure or other Contract Pause Condition. No same-Run retry, fallback, automatic replacement or V2.
- R2 cell 04 is a one-time recorded orchestration exception: its outer runner returned `124`, its product-process exit code is unknown, but its immutable product evidence and Main's repeated Inspector/aggregate checks are terminal, integrity-valid, comparable and usage-complete. Never rerun or replace cell 04. Continue only from cell 05, preserve the timeout limitation in final claims, use an outer command-runner wait of at least 900000 ms without changing Manifest product budgets, and stop on any further post-side-effect launcher anomaly.
