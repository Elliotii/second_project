...
This repository is building an Agent Harness Reliability Workbench. V0 is complete and accepted. V1-A is closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; V1-B remains historically closed inconclusive; and V1-C is closed and accepted with `PASS_V1_C_COMPARISON_COMPLETION`. The valid 24-cell R2 Pilot produced A 7/8, B Skill-only 8/8 and C Skill + Runtime Control 7/8, with zero invalids, eight passing fairness blocks and one unsuccessful bounded C Recovery. Main promotes Skill-only only within the frozen protocol and rejects the current same-Session Runtime Control treatment as the V1 default; this is not a universal Skill/Runtime claim. R2 cell 04 remains disclosed with outer exit `124` and unknown product exit, but complete terminal product evidence and no retry/replacement. No Goal is active. V2 bounded multi-path recovery remains the Portfolio North Star but is unauthorized; the bounded Pi SDK/Extension compatibility checkpoint should be reviewed before any V2 Contract.
