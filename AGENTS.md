...
- V3.5 is governed by accepted `V3_5_CHARTER.md`. Goal 1 is closed and accepted while preserving `Session != Run`: public Pi JSONL is resumable conversation/context authority; Workbench Run artifacts remain execution/evaluation authority; the single-writer catalog is navigation metadata only. Full raw Session remains local Runtime state while CLI/API/UI/report consumers receive only allowlisted safe projections. The accepted claim is settled cross-process reopen/continue with deterministic/Faux evidence, not crash recovery, exactly-once Tool effects, database durability or external-model behavior. Automatic catalog rebuild remains a recorded productization limitation; missing/corrupt catalog currently fails closed. Goal 2 is closed inconclusive: its `ed2dc14...` implementation baseline is preserved, but the sole real Base exhausted its 16-request cap after reaching correct workspace bytes, before settlement, external Verifier or Candidate. Do not claim a valid pair or Skill effect. Goal 2.5 is active under `V3_5_G2_5_GOAL_CONTRACT.md`. Its initial new top-level Implementation Session is zero-access and may change only the Contract allowlist: model-visible legal command IDs, successful `public_test` termination through Pi's public Tool result, strict pre-dispatch budget terminal/quiescence, orthogonal trajectory/task outcomes, the secondary accounting fix, focused tests and reports. Main must review the zero-access Candidate, one fresh focused Audit Session may inspect only Tool termination, budget-stop quiescence, Verifier handoff and evidence/fairness, and any finding returns to the original Implementation Session. Only an exact audited Execution Baseline may unlock the single Base/Candidate real pair under the frozen Goal 2 inputs and budgets. Retry, fallback, replacement, extra arm/Case, Pi patch/private import and SDK/Extension/RPC/server switching are forbidden. Goal 3 and final V3.5 acceptance remain unauthorized.
