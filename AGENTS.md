...
This repository is building an Agent Harness Reliability Workbench. V0-A, V0-B and V0-C are closed and accepted, so the minimal usable V0 Workbench is complete. V1-A is closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; its independently re-audited Implementation Baseline is `784bd1ec06c2aa9ed554a7da661bdf582097bcdf`. V1-B is historically closed as `CLOSE_V1_B_INCONCLUSIVE_AUTHORIZED_SEQUENCE_EXHAUSTED`, not completed, with zero valid comparable Runs. V1-C is now the active Goal under the accepted `V1_C_GOAL_CONTRACT.md`. Its first stage is a dedicated zero-real-call correction of the project-owned request-cap/synthetic-failure attribution boundary. Stage 1 permits only the Contract allowlist and deterministic tests, with zero credential reads, network, external-provider/model calls, dependency installation, Pi changes/private imports, control-state edits or dedicated-session Git commits. Candidate Commit, focused audit, audited Execution Baseline, real Canary and full A/B/C Pilot remain separate unauthorized control points. V1-B evidence remains immutable; V1-C must use new identities and cannot retroactively terminalize V1-B. V2 remains the Portfolio North Star and is not authorized.
