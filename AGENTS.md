...
This repository is building an Agent Harness Reliability Workbench. V0-A, V0-B and V0-C are closed and accepted, so the minimal usable V0 Workbench is complete. V1-A is closed and accepted with `PASS_V1_A_DETERMINISTIC_SUBSTRATE`; its independently re-audited Implementation Baseline is `784bd1ec06c2aa9ed554a7da661bdf582097bcdf`. V1-B is the active Goal under the accepted `V1_B_GOAL_CONTRACT.md`. Stage 1 completed on corrected Control Baseline `de75ca7a4d5376713f01ca475bc5ad7637c70443`; corrected Candidate `a11690e5827d9d540b731156799566bea21c689e` passed focused independent re-audit with both original P1 findings closed and no new blocking finding. The user has authorized the deterministic Execution Baseline, a fresh no-source-edit Stage 2 Session, opaque `DEEPSEEK_API_KEY` resolution, network and real DeepSeek calls under the immutable 24-cell Pilot and whole-Pilot USD 2 hard cap, plus V1-B closeout commit. Stage 2 must stop on the Contract and bounded-autonomy exit conditions; it may not edit source, fixtures, Manifest, tests, control state, Pi, reference material or Git. V1 compares Baseline, Skill-only and Skill + External Verifier / Runtime Control without presuming the winner; V2 bounded multi-path recovery remains the Portfolio North Star and is not authorized.
