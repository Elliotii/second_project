...
test("V1-B post-audit CLI requires explicit Stage 2 authority before Pilot initialization", () => {
	...
	result = spawnSync(process.execPath, [resolve(PROJECT_ROOT, "workbench/src/cli.ts"), "v1b", "run-next", "--pilot-root", stage1Pilot, "--stage2-real-authority"], { cwd: PROJECT_ROOT, encoding: "utf8" });
...
test("V1-B tracked Stage 2 CLI fails sanitized and without dispatch when DEEPSEEK_API_KEY is absent", () => {
	...
	assert.equal(result.status, 1); assert.equal(result.stdout, ""); assert.match(result.stderr, /FixedProviderBoundaryErrorV1B: V1-B fixed provider boundary failed/); assert.doesNotMatch(result.stderr, /real execution dependencies are unavailable/);
	...
	assert.deepEqual(journal.map((entry) => entry.type), ["run_started", "workspace_materialized", "attempt_started"]);
