...
import { createWorkflowRegistrationV37, loadWorkflowRegistrationV37 } from "../src/v37/workflow-registration-v37.ts";
...
test("registry, Manifest, Envelope and hardlink mutation fail closed", () => {
	for (const variant of ["registry-key", "manifest", "envelope", "hardlink"] as const) {
		...
		assert.throws(() => loadRegisteredCaseFromHostRegistryV37({ projectRoot: root, caseId: CASE_ID }), /exact-key|digest|identity|mismatch|ordinary singly linked|approval/i, variant);
	}
});

test("disabled registration blocks new authority but permits explicit historical read-only load", () => {
	const root = cloneHost("disabled");
	const registryPath = resolve(root, "workbench/config/v37/registered-cases/registry-v1.json");
	const registry = readJson<any>(registryPath);
	const first = readJson<any>(resolve(root, registry.entries[0].envelope_locations[0]));
	...
	const secondLocation = "workbench/config/v37/registered-cases/envelopes/v37-g1-det-recovery.r2.json";
	writeJson(resolve(root, secondLocation), second);
	...
	assert.throws(() => loadRegisteredCaseFromHostRegistryV37({ projectRoot: root, caseId: CASE_ID }), /disabled/);
	const historical = loadRegisteredCaseFromHostRegistryV37({ projectRoot: root, caseId: CASE_ID, allowDisabledHistorical: true });
	assert.equal(historical.historical_read_only, true);
});
...
test("package tamper, confirmation replay and source mutation fail independent reopen", () => {
	...
	assert.match(sourceInspected.errors.join("; "), /source|artifact|digest|recovery|integrity|recomputation/i);
});
