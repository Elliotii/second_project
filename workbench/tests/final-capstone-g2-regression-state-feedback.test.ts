...
async function rollbackReady(label: string) {
	const sequence = await setupAssessmentSequence(label); const admission = await boundAdmission(sequence, "failed"); const comparisonRoot = resolve(sequence.root, "comparison");
	const port: FauxValidationPortV3 = { execute: async ({ state, workspaceRoot }) => { if (state.state_digest === sequence.initialActive.state_digest) writeFileSync(resolve(workspaceRoot, "subject.txt"), "fixed\n"); return { settled: true, events: [] }; } };
	await executeAcceptedStateRegressionComparisonG2({ projectRoot: PROJECT_ROOT, runRoot: comparisonRoot, comparisonId: `${label}-comparison`, projectId: PROJECT_ID, admissionRoot: admission.admissionRoot, registrationPath: admission.registrationPath, admissionId: admission.admissionId, stateRoot: sequence.stateRoot, expectedActive: sequence.promotedActive, sourceWorkspaceRoot: sequence.workspace, task: sequence.primary.task, verifier: sequence.primary, ...PROFILE, immutableBasePrompt: SYSTEM_PROMPT, immutableBasePromptSha256: SYSTEM_PROMPT_SHA256, port });
	const context = assessmentContext(sequence, admission, comparisonRoot, sequence.initialActive.state_digest); const assessed = await persistStateAssessmentG2(context); assert.equal(assessed.assessment.assessment_result, "rollback");
	return { sequence, admission, comparisonRoot, context, assessed };
}
...
test("future assessment roots reject intermediate junctions and existing assessment hardlinks fail idempotent reopen", async () => {
	const sequence = await setupAssessmentSequence("path-integrity"); const admission = await boundAdmission(sequence, "passed"); const context = assessmentContext(sequence, admission, null, null); const assessed = await persistStateAssessmentG2(context);
	const assessmentPath = resolve(context.assessmentRoot, `assessments/${assessed.assessment.assessment_id}.json`); const hardlink = resolve(context.assessmentRoot, "assessment-hardlink.json"); linkSync(assessmentPath, hardlink); await assert.rejects(persistStateAssessmentG2(context), /ordinary singly linked/); rmSync(hardlink);
	const junctionTarget = resolve(sequence.root, "junction-target"); const junctionParent = resolve(sequence.root, "junction-parent"); mkdirSync(junctionTarget); symlinkSync(junctionTarget, junctionParent, "junction"); const redirectedRoot = resolve(junctionParent, "assessment-root");
	await assert.rejects(persistStateAssessmentG2({ ...context, assessmentRoot: redirectedRoot }), /symlink|junction|real path/); assert.equal(existsSync(resolve(junctionTarget, "assessment-root")), false);
});

test("post-mutation application obstruction recovers one exact V3 rollback Decision, remains idempotent, and rejects hardlinked artifacts", async () => {
	const ready = await rollbackReady("recovery"); const { sequence, context, assessed } = ready; const applicationsPath = resolve(context.assessmentRoot, "applications"); writeFileSync(applicationsPath, "obstruction\n");
	const before = await inspectStateStoreV3({ stateRoot: sequence.stateRoot, expectedProjectId: PROJECT_ID, immutableBasePrompt: SYSTEM_PROMPT, immutableBasePromptSha256: SYSTEM_PROMPT_SHA256 }); await assert.rejects(applyAssessedRollbackG2({ ...context, assessmentId: assessed.assessment.assessment_id }), /non-directory|EEXIST/);
	const interrupted = await inspectStateStoreV3({ stateRoot: sequence.stateRoot, expectedProjectId: PROJECT_ID, immutableBasePrompt: SYSTEM_PROMPT, immutableBasePromptSha256: SYSTEM_PROMPT_SHA256 }); assert.equal(interrupted.integrity_valid, true, interrupted.errors.join("; ")); assert.equal(interrupted.decisions.length, before.decisions.length + 1); assert.equal(interrupted.active!.state_digest, sequence.initialActive.state_digest); assert.equal(interrupted.active!.binding_revision, sequence.promotedActive.binding_revision + 1); assert.equal(existsSync(resolve(context.assessmentRoot, `authorizations/${assessed.assessment.assessment_id}.json`)), true); assert.equal(existsSync(resolve(context.assessmentRoot, `applications/${assessed.assessment.assessment_id}.json`)), false); assert.equal((await inspectFinalCapstoneG2({ ...context, assessmentId: assessed.assessment.assessment_id })).integrity_valid, false);
	rmSync(applicationsPath); const recovered = await applyAssessedRollbackG2({ ...context, assessmentId: assessed.assessment.assessment_id }); assert.equal(recovered.idempotent_existing, false); const afterRecovery = await inspectStateStoreV3({ stateRoot: sequence.stateRoot, expectedProjectId: PROJECT_ID, immutableBasePrompt: SYSTEM_PROMPT, immutableBasePromptSha256: SYSTEM_PROMPT_SHA256 }); assert.equal(afterRecovery.decisions.length, interrupted.decisions.length); assert.equal(recovered.application.v3_rollback_decision_id, interrupted.active!.decision_id); assert.equal((await inspectFinalCapstoneG2({ ...context, assessmentId: assessed.assessment.assessment_id })).integrity_valid, true);
	const again = await applyAssessedRollbackG2({ ...context, assessmentId: assessed.assessment.assessment_id }); assert.equal(again.idempotent_existing, true); assert.equal(again.application.application_digest, recovered.application.application_digest);
	const script = `import { inspectFinalCapstoneG2 as inspect } from ${JSON.stringify(new URL("../src/inspect-final-capstone-g2.ts", import.meta.url).href)}; const result=await inspect(JSON.parse(process.argv[1])); console.log(JSON.stringify(result)); process.exit(result.integrity_valid?0:1);`; const reopened = spawnSync(process.execPath, ["--input-type=module", "-e", script, JSON.stringify({ ...context, assessmentId: assessed.assessment.assessment_id })], { encoding: "utf8" }); assert.equal(reopened.status, 0, reopened.stderr);
	const authorizationPath = resolve(context.assessmentRoot, `authorizations/${assessed.assessment.assessment_id}.json`); const authorizationLink = resolve(context.assessmentRoot, "authorization-hardlink.json"); linkSync(authorizationPath, authorizationLink); await assert.rejects(applyAssessedRollbackG2({ ...context, assessmentId: assessed.assessment.assessment_id }), /ordinary singly linked/); rmSync(authorizationLink);
	const applicationPath = resolve(context.assessmentRoot, `applications/${assessed.assessment.assessment_id}.json`); const applicationLink = resolve(context.assessmentRoot, "application-hardlink.json"); linkSync(applicationPath, applicationLink); await assert.rejects(applyAssessedRollbackG2({ ...context, assessmentId: assessed.assessment.assessment_id }), /ordinary singly linked/); rmSync(applicationLink);
	writeFileSync(resolve(sequence.root, "correction-1-recovery-summary.json"), `${stableJson({ schema_version: 1, assessment_id: assessed.assessment.assessment_id, authorization_id: recovered.authorization.authorization_id, application_id: recovered.application.application_id, v3_rollback_decision_id: recovered.application.v3_rollback_decision_id, decision_count_before: before.decisions.length, decision_count_after_interruption: interrupted.decisions.length, decision_count_after_recovery: afterRecovery.decisions.length, recovered_without_second_rollback: afterRecovery.decisions.length === interrupted.decisions.length, active_after_recovery: active(afterRecovery.active!), zero_access: { credential_reads: 0, network_calls: 0, external_provider_calls: 0, real_model_calls: 0 } })}\n`);
});

test("recovery rejects conflicting canonical bytes, orphan pre-mutation authorization, and later pointer movement", async () => {
	const completed = await rollbackReady("recovery-conflict"); const applied = await applyAssessedRollbackG2({ ...completed.context, assessmentId: completed.assessed.assessment.assessment_id }); const authorizationPath = resolve(completed.context.assessmentRoot, `authorizations/${completed.assessed.assessment.assessment_id}.json`); const applicationPath = resolve(completed.context.assessmentRoot, `applications/${completed.assessed.assessment.assessment_id}.json`); const authorizationBytes = readFileSync(authorizationPath, "utf8"); const applicationBytes = readFileSync(applicationPath, "utf8");
	const badAuthorization = JSON.parse(authorizationBytes); badAuthorization.authorization_digest = "0".repeat(64); writeFileSync(authorizationPath, `${stableJson(badAuthorization)}\n`); await assert.rejects(applyAssessedRollbackG2({ ...completed.context, assessmentId: completed.assessed.assessment.assessment_id }), /authorization is invalid|linkage is invalid/); writeFileSync(authorizationPath, authorizationBytes);
	const badApplication = JSON.parse(applicationBytes); badApplication.application_digest = "0".repeat(64); writeFileSync(applicationPath, `${stableJson(badApplication)}\n`); await assert.rejects(applyAssessedRollbackG2({ ...completed.context, assessmentId: completed.assessed.assessment.assessment_id }), /linkage is invalid/); assert.equal((await inspectFinalCapstoneG2({ ...completed.context, assessmentId: completed.assessed.assessment.assessment_id })).integrity_valid, false); writeFileSync(applicationPath, applicationBytes); assert.equal(applied.application.application_digest, JSON.parse(applicationBytes).application_digest);
	const orphan = await rollbackReady("recovery-orphan"); const assessment = orphan.assessed.assessment; const authorizationSeed = { assessment_id: assessment.assessment_id, assessment_digest: assessment.assessment_digest, expected_active: assessment.bound_state, target_state_digest: assessment.rollback_target_digest }; const authorizationBody = { schema_version: 1, authorization_id: `rollback-authorization-${digestObject(authorizationSeed).slice(0, 32)}`, project_id: assessment.project_id, assessment_id: assessment.assessment_id, assessment_digest: assessment.assessment_digest, expected_active: assessment.bound_state, target_state_digest: assessment.rollback_target_digest, authority: "host_assessed_rollback" }; const authorization = { ...authorizationBody, authorization_digest: digestObject(authorizationBody) }; mkdirSync(resolve(orphan.context.assessmentRoot, "authorizations")); writeFileSync(resolve(orphan.context.assessmentRoot, `authorizations/${assessment.assessment_id}.json`), `${stableJson(authorization)}\n`); const orphanPointer = readFileSync(resolve(orphan.sequence.stateRoot, "active.json")); await assert.rejects(applyAssessedRollbackG2({ ...orphan.context, assessmentId: assessment.assessment_id }), /exactly one matching/); assert.deepEqual(readFileSync(resolve(orphan.sequence.stateRoot, "active.json")), orphanPointer);
	const concurrent = await rollbackReady("recovery-concurrent"); const obstruction = resolve(concurrent.context.assessmentRoot, "applications"); writeFileSync(obstruction, "obstruction\n"); await assert.rejects(applyAssessedRollbackG2({ ...concurrent.context, assessmentId: concurrent.assessed.assessment.assessment_id }), /non-directory|EEXIST/); rmSync(obstruction); const afterFirst = await inspectStateStoreV3({ stateRoot: concurrent.sequence.stateRoot, expectedProjectId: PROJECT_ID, immutableBasePrompt: SYSTEM_PROMPT, immutableBasePromptSha256: SYSTEM_PROMPT_SHA256 }); await rollbackActiveStateV3({ stateRoot: concurrent.sequence.stateRoot, projectId: PROJECT_ID, targetStateDigest: concurrent.sequence.promotedActive.state_digest, expectedActive: active(afterFirst.active!), immutableBasePrompt: SYSTEM_PROMPT, immutableBasePromptSha256: SYSTEM_PROMPT_SHA256 }); await assert.rejects(applyAssessedRollbackG2({ ...concurrent.context, assessmentId: concurrent.assessed.assessment.assessment_id }), /unrelated or later active pointer movement/);
});
