...
const authority=()=>({schema_version:1 as const,kind:"v37_g3b_host_execution_port_bridge_authority" as const,case_id:CASE_ID as typeof CASE_ID,project_id:"v37-real-recovery-project" as const,configuration_candidate_commit:"ca33885f4691f00ab9ab90643f8ed5fbc0825bb8" as const,configuration_candidate_tree:"ae12fd560f1d6dc802648a682eb5d94f96335f80" as const,manifest_body_digest:"dcaf5ff929a5a98ae32fe42b7f1700cf4a7dd0630e2989b10b8fd5fb219a39dc" as const,workflow_registration_digest:"1315d746ccbcad5927a6163319f342cb836b2d6dc07389eb5d44f1fb28ea6245" as const,follow_up_execution_profile_digest:"89fa6b4227495ddd1d972c91f1139532858842c4e3a13529d682b99aa93b7ace" as const,registry_index_digest:"665e911654eb811337ede1ce11281fbe167acba2e52e9b532c53270290d14ee3" as const,candidate_proposal_authority_digest:"c93ce9bb18b23990f909c24ea2fdf13d0f6b9b6b93f7401b0f8aef22bbaf6450" as const,regression_authority_digest:"91497eba6a828e2845ca22145669926e0debbe4ca0aa5f1b67a06fb12d5750ab" as const});

let modelFactoryCalls=0;
let scenario:"complete"|"initial_pass"|"malformed"|"invalid_schema"|"over_budget"|"unknown_usage"|"primary_abort_after_evidence"="complete";
...
mock.module("../src/pi/pi-run-handle-v2b.ts",{namedExports:{createRealExecutionPortV2B:(options:any)=>{
	...
	return {execute:async(input:any)=>{if(!opened){opened=true;const access=options.authority.open();await access.resolveCredential();options.realCounters.credential_reads++;}
		const role=ordinal===0?"primary":ordinal===1?"continue_failed_session":"fresh_session_from_failure_seed";ordinal++;options.onAttemptStarted?.({attemptId:input.attemptId,role});const result=await delegate.execute(scenario==="initial_pass"&&role==="primary"?{...input,mode:"primary_pass"}:input);const metadata=await input.session.getMetadata();options.realCounters.network_calls+=result.providerDispatches;options.realCounters.external_provider_calls+=result.providerDispatches;options.realCounters.real_model_calls+=result.providerDispatches;
...
test.beforeEach(()=>{for(const path of [".runs/v37/g3a-product",".runs/v37/g3b/state-stores",".runs/v37/g3b/host-bridge",".runs/v37/host-authority/v37-g3a-host-registry-v2"])rmSync(resolve(PROJECT_ROOT,path),{recursive:true,force:true});modelFactoryCalls=0;scenario="complete";capturedCandidateTemplates=[];runtimeCreateGate=null;signalRuntimeCreate=null;});
...
test("real Primary initial pass completes only the Primary unit and closes the workflow path honestly",async()=>{
	scenario="initial_pass";const bridge=createRealExecutionPortBridgeV37G3B(PROJECT_ROOT,authority(),{resolve:async()=>"test-opaque"});let model=await bridge.service.createWorkflow(CASE_ID);model=await bridge.service.act(model.workflow_id,"run_primary");assert.equal(model.stage,"no_recovery_needed");assert.equal(model.receipt_count,1);assert.deepEqual(model.available_actions,[]);const inspection=bridge.inspect();assert.deepEqual(inspection.completed_units.map((item)=>item.unit),["primary"]);assert.equal(inspection.in_flight,null);assert.equal(inspection.state,"open");assert.equal(inspection.completed_units[0]?.usage.commands,1);await bridge.close();
});
