...
import { assertBoundedEditBudgetProfileV36, type BoundedEditBudgetProfileV36, V36_64_REQUEST_BOUNDED_EDIT_BUDGET_PROFILE, V36_DAILY_BOUNDED_EDIT_BUDGET_PROFILE, V36G2_FROZEN_BOUNDED_EDIT_BUDGET_PROFILE } from "../v36/budget-profile-v36.ts";
...
function parseManifestG2(value: unknown): RuntimeManifestG2V36 {
	...
	const providerRequestsMax = registered ? V36_64_REQUEST_BOUNDED_EDIT_BUDGET_PROFILE.provider_requests_hard_max : V36_DAILY_BOUNDED_EDIT_BUDGET_PROFILE.provider_requests_hard_max;
	...
	if ((!registered && manifest.schema_version !== 2) || manifest.mode !== "v36_interactive_bounded_edit" || !ID.test(manifest.run_id) || !ID.test(manifest.session_id) || !ID.test(manifest.project_id) || !ID.test(manifest.workspace_id) || !SHA256.test(manifest.session_pin_digest) || manifest.settled !== true || !SHA256.test(manifest.prior_context_sha256) || manifest.prior_context_sha256 !== manifest.provider_observed_prior_context_sha256 || !SHA256.test(manifest.session_entries_sha256_after_turn) || !SHA256.test(manifest.prompt_sha256) || !Number.isSafeInteger(manifest.provider_requests) || manifest.provider_requests < 1 || manifest.provider_requests > providerRequestsMax || !Array.isArray(manifest.active_tool_names) || stableJson(manifest.active_tool_names) !== stableJson(["workspace_read", "workspace_list", "workspace_search", "workspace_edit", "workspace_write", "run_command"]) || !Array.isArray(manifest.tool_call_ids) || stableJson(manifest.tool_call_ids) !== stableJson(manifest.tool_result_ids) || ![manifest.credential_reads, manifest.network_calls, manifest.external_provider_calls, manifest.real_model_calls, manifest.project_command_executions, manifest.docker_project_command_executions, manifest.input_tokens, manifest.output_tokens].every((entry) => Number.isSafeInteger(entry) && entry >= 0) || !commandIdentityValid || !SHA256.test(manifest.workspace_identity_after) || !Number.isFinite(manifest.cost_usd) || manifest.cost_usd < 0 || !SHA256.test(manifest.manifest_digest) || digestObject(body) !== manifest.manifest_digest) throw new Error("V3.6 Goal 2 Runtime Manifest is invalid");
	return manifest;
}
...
function parseBudgetStopTerminal(value: unknown): ProviderRequestBudgetTerminalV36 {
	...
	const namedBudget = terminal.budget_profile_id === V36_DAILY_BOUNDED_EDIT_BUDGET_PROFILE.profile_id ? V36_DAILY_BOUNDED_EDIT_BUDGET_PROFILE : terminal.budget_profile_id === V36_64_REQUEST_BOUNDED_EDIT_BUDGET_PROFILE.profile_id ? V36_64_REQUEST_BOUNDED_EDIT_BUDGET_PROFILE : null;
	const dailyBudget = terminal.schema_version === 2 && namedBudget !== null && terminal.request_attempts === namedBudget.provider_requests_hard_max + 1 && terminal.provider_dispatches === namedBudget.provider_requests_hard_max && terminal.provider_responses === namedBudget.provider_requests_hard_max && terminal.provider_requests_max === namedBudget.provider_requests_hard_max;
	if (
...
function finiteBudgetProfile(profileId: ReconciledFiniteBudgetTerminalV36["budget_profile_id"]): BoundedEditBudgetProfileV36 {
	const profile = profileId === V36G2_FROZEN_BOUNDED_EDIT_BUDGET_PROFILE.profile_id ? V36G2_FROZEN_BOUNDED_EDIT_BUDGET_PROFILE : profileId === V36_DAILY_BOUNDED_EDIT_BUDGET_PROFILE.profile_id ? V36_DAILY_BOUNDED_EDIT_BUDGET_PROFILE : profileId === V36_64_REQUEST_BOUNDED_EDIT_BUDGET_PROFILE.profile_id ? V36_64_REQUEST_BOUNDED_EDIT_BUDGET_PROFILE : null;
...
function parseFiniteBudgetStopTerminal(value: unknown): ReconciledFiniteBudgetTerminalV36 {
	...
	if (
		(terminal.schema_version !== 3 && terminal.schema_version !== 4) || !["v36g2_frozen_acceptance_v1", "v36_daily_bounded_edit_v2", "v36_64_request_bounded_edit_v3"].includes(terminal.budget_profile_id) || terminal.terminal_kind !== "v36_reconciled_finite_budget_terminal" || terminal.trajectory_outcome !== "finite_budget_terminal" || (!accountedUsage && !toolStop && !wallStop) ||
	...
	async executeBoundedTurn(options: {
		...
		if (terminalError !== null) {
			...
			const terminalBody: Omit<ProviderRequestBudgetTerminalV36, "terminal_digest"> = {
				schema_version: options.budgetProfile.profile_id === V36G2_FROZEN_BOUNDED_EDIT_BUDGET_PROFILE.profile_id ? 1 : 2,
				...(options.budgetProfile.profile_id === V36G2_FROZEN_BOUNDED_EDIT_BUDGET_PROFILE.profile_id ? {} : { budget_profile_id: options.budgetProfile.profile_id }),
