...
class SharedCredentialLease {
	private value:string|undefined; private reads=0; private closed=false;
	...
	async resolve():Promise<string>{if(this.closed)throw new Error("bridge credential lease is closed");if(this.value!==undefined)return this.value;const value=await this.resolver.resolve();if(typeof value!=="string"||value.length===0)throw new Error("opaque Credential resolution failed");this.value=value;this.reads++;return value;}
	count():number{return this.reads;} clear():void{this.value=undefined;this.closed=true;}
}
...
class Coordinator {
	...
	initialize(authority:RealExecutionPortBridgeAuthorityV37G3B):void{mkdirSync(this.root,{recursive:true});writeOnceJson(this.root,"construction.json",{schema_version:1,kind:"v37_g3b_bridge_construction",bridge_id:this.bridgeId,case_id:CASE_ID,authority_digest:digestObject(authority)});}
	...
	expect(unit:Unit):void{this.assertOpen();const expected=UNIT_ORDER[this.units.length];if(expected!==unit){this.fail(unit,new Error(`bridge unit order invalid: expected ${expected??"none"}`));throw new Error(`bridge unit order invalid: expected ${expected??"none"}`);}}
	...
	complete(unit:Unit,usage:Usage,evidence:unknown):void{this.assertOpen();if(UNIT_ORDER[this.units.length]!==unit)throw new Error("bridge unit completion order invalid");this.validateUsage(unit,usage);const item:UnitInspection={unit,status:"complete",usage:{...usage},evidence_digest:digestObject(evidence)};this.units.push(item);writeOnceJson(this.root,`units/${String(this.units.length).padStart(2,"0")}-${unit}.json`,item);}
	fail(unit:Unit,error:Error):never{this.state="faulted";const body={schema_version:1,kind:"v37_g3b_bridge_failure",bridge_id:this.bridgeId,unit,error_sha256:sha256(error.message)};try{writeOnceJson(this.root,"failure.json",body);}catch{}throw error;}
	inspection():RealExecutionPortBridgeInspectionV37G3B{const body={bridge_id:this.bridgeId,case_id:CASE_ID as typeof CASE_ID,state:this.state,completed_units:structuredClone(this.units),totals:sumUsage(this.units.map((v)=>v.usage)),real_access:{...this.real},credential_resolution_count:this.lease.count()};return {...body,inspection_digest:digestObject(body)};}
...
export function createRealExecutionPortBridgeV37G3B(projectRoot:string,authorityToken:RealExecutionPortBridgeAuthorityV37G3B,opaqueCredentialResolver:OpaqueCredentialResolverV1):RealExecutionPortBridgeV37G3B {
	...
	const loaded=loadRegisteredCaseFromHostRegistryV37G3A({projectRoot,caseId:CASE_ID});const declaration=primaryExecutionDeclarationV37G3A(loaded.manifest),derived=constructionAuthorityDigestsV37G3A(loaded.manifest),follow=followUpAccessExpectationV37G3A(loaded.follow_up_execution_profile);
