...
const UNIT_CAPS: Readonly<Record<Unit, { requests:number; tokens:number; tools:number; commands:number; wallMs:number; cost:number }>> = Object.freeze({
	primary:{requests:V2B_ATTEMPT_CAPS.provider_requests,tokens:V2B_ATTEMPT_CAPS.tokens,tools:V2B_ATTEMPT_CAPS.tool_calls,commands:64,wallMs:V2B_ATTEMPT_CAPS.active_execution_time_ms,cost:0.2},
	recovery_a:{requests:V2B_ATTEMPT_CAPS.provider_requests,tokens:V2B_ATTEMPT_CAPS.tokens,tools:V2B_ATTEMPT_CAPS.tool_calls,commands:64,wallMs:V2B_ATTEMPT_CAPS.active_execution_time_ms,cost:0.2},
	recovery_b:{requests:V2B_ATTEMPT_CAPS.provider_requests,tokens:V2B_ATTEMPT_CAPS.tokens,tools:V2B_ATTEMPT_CAPS.tool_calls,commands:64,wallMs:V2B_ATTEMPT_CAPS.active_execution_time_ms,cost:0.2},
...
const GLOBAL_CAP = Object.freeze({requests:385,tokens:3162112,tools:576,commands:195,wallMs:21720000,cost:1.4});
...
class Coordinator {
	...
	finishPrimaryPass():void{if(this.reservation?.group!=="primary_recovery"||this.reservation.unit!=="primary"||this.units.length!==1||this.units[0]?.unit!=="primary")this.fail("primary",new Error("Primary PASS reservation release mismatch"));this.reservation=null;}
...
function primaryPort(coordinator:Coordinator,lease:SharedCredentialLease):ProductPrimaryExecutionPortV37G3A{return {async execute(request){
	...
	try{
		...
		const terminal=await executeRunV2A({projectRoot:request.projectRoot,runRoot:request.runRoot,runId:request.runId,taskId:String((request.loadedCase.manifest.primary_task_spec.body as {task_id:string}).task_id),primaryMode:"fail",candidateModes:["pass","pass"],executionPort:port,realExecutionAuthorized:true,realCallCounters:coordinator.real});const initialPass=(terminal as {outcome?:unknown}).outcome==="initial_pass";const expectedAttempts=initialPass?1:3;if(evidence.length!==expectedAttempts||roles.length!==expectedAttempts)coordinator.fail("primary",new Error(initialPass?"Primary PASS did not produce exactly one attempt":"Primary V2 recovery group did not produce exact three attempts"));for(let i=0;i<evidence.length;i++){const item=evidence[i]!,unit=UNIT_ORDER[i]!;const usage:Usage={provider_requests:item.usage.provider_requests,combined_tokens:item.usage.tokens,tool_calls:item.usage.tool_calls,commands:commandCalls(item.composition.session_path),wall_time_ms:item.usage.active_execution_time_ms,cost_usd:item.usage.real_cost_usd};coordinator.complete(unit,usage,{attempt_id:item.attempt_id,role:item.role,terminal_reason:item.terminal_reason,usage,runtime_composition_id:RUNTIME_COMPOSITIONS[0].composition_id});}if(initialPass)coordinator.finishPrimaryPass();return terminal as unknown as Record<string,unknown>;
