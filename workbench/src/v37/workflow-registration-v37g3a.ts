...
import type { PrimaryRunBindingV37, TaskInstanceV37, WorkflowRegistrationV37 } from "../contracts/v37-types.ts";
...
function deriveWorkflow(loaded: LoadedRegisteredCaseV37G3A, workflowId: string, createdAt: string): WorkflowRegistrationV37 {
...
export function createWorkflowRegistrationV37G3A(options: { projectRoot: string; dataRoot: string; caseId: string; workflowId: string; createdAt: string }): { workflow: WorkflowRegistrationV37; primary: TaskInstanceV37; follow_up: TaskInstanceV37 } {
	if (!ID.test(options.workflowId) || Number.isNaN(Date.parse(options.createdAt))) throw new Error("workflow ID/timestamp invalid");
	const loaded = loadRegisteredCaseFromHostRegistryV37G3A({ projectRoot: options.projectRoot, caseId: options.caseId });
...
export function loadWorkflowRegistrationV37G3A(options: { projectRoot: string; dataRoot: string; workflowId: string; allowHistoricalReadOnly?: boolean }): { loadedCase: LoadedRegisteredCaseV37G3A; workflow: WorkflowRegistrationV37; primary: TaskInstanceV37; follow_up: TaskInstanceV37 } {
	...
	const historicalTrustRootDigest = deriveRegistryTrustRootDigestV37G3A(loadedCase, pinnedEnvelopeCount);
...
export function bindPrimaryRunV37G3A(options: { projectRoot: string; dataRoot: string; workflowId: string; runId: string; runRoot: string; boundAt: string }): PrimaryRunBindingV37 {
...
export function loadPrimaryRunBindingV37G3A(options: { projectRoot: string; dataRoot: string; workflowId: string; runRoot: string; allowHistoricalReadOnly?: boolean }): PrimaryRunBindingV37 {
	const registered = loadWorkflowRegistrationV37G3A({ projectRoot: options.projectRoot, dataRoot: options.dataRoot, workflowId: options.workflowId, allowHistoricalReadOnly: options.allowHistoricalReadOnly });
...
export function v37G3ADataRootPath(projectRoot: string, dataRoot: string): string {
