...
function ordinaryJson<T>(path: string): T { const stats=lstatSync(path); if(!stats.isFile()||stats.isSymbolicLink()||stats.nlink!==1) throw new Error("journal artifact must be ordinary singly linked file"); const bytes=readFileSync(path,"utf8"); const value=JSON.parse(bytes) as T; if(bytes!==`${stableJson(value)}\n`) throw new Error("journal artifact bytes are not canonical"); return value; }
...
export function loadWorkflowJournalV37G3A(options:{projectRoot:string;dataRoot:string;workflowId:string}):{header:WorkflowHeaderV37G3A;receipts:WorkflowTransitionReceiptV37G3A[]} {
	...
	const header=ordinaryJson<WorkflowHeaderV37G3A>(resolve(journal,"header.json"));
	...
	const receipts=names.map((name,index)=>{ const receipt=ordinaryJson<WorkflowTransitionReceiptV37G3A>(resolve(receiptRoot,name)); if(receipt.workflow_id!==options.workflowId||receipt.sequence!==index+1||receipt.previous_receipt_digest!==prior||digestObject(receiptBody(receipt))!==receipt.receipt_digest) throw new Error("workflow journal receipt chain invalid"); prior=receipt.receipt_digest; return receipt; });
