...
import { loadRegisteredFollowUpExecutionProfileV37G3A } from "./follow-up-execution-profile-v37g3a.ts";
import { followUpAccessExpectationV37G3A } from "./host-registry-v37g3a.ts";
import { loadWorkflowRegistrationV37G3A, v37G3ADataRootPath } from "./workflow-registration-v37g3a.ts";
import { recomputeRegisteredRecoveryAdmissionV37G3A } from "./registered-recovery-v37g3a.ts";

const ID = /^[a-z0-9][a-z0-9._:-]{0,127}$/;
const SHA256 = /^[a-f0-9]{64}$/;
const INSPECTOR_ID = "inspect-v37g2-registered-follow-up-v1" as const;

export interface RegisteredFollowUpOptionsV37G3A {
	projectRoot: string;
	dataRoot: string;
	workflowId: string;
...
function workflowFollowUpRoot(options: Pick<RegisteredFollowUpOptionsV37G3A, "projectRoot" | "dataRoot" | "workflowId">, create: boolean): string {
	if (!ID.test(options.workflowId)) throw new Error("follow-up workflow identity invalid");
	const dataRoot = v37G3ADataRootPath(options.projectRoot, options.dataRoot);
...
export function registeredFollowUpPromotionValidationRootV37G3A(options: Pick<RegisteredFollowUpOptionsV37G3A, "projectRoot" | "dataRoot" | "workflowId">): string {
	if (!ID.test(options.workflowId)) throw new Error("follow-up workflow identity invalid");
	const dataRoot = v37G3ADataRootPath(options.projectRoot, options.dataRoot);
...
async function inspectRegisteredPromotionEvidenceV37(
	options: RegisteredFollowUpOptionsV37G3A,
	...
	const runRoot = registeredFollowUpPromotionValidationRootV37G3A(options);
...
function recoveryAdmission(options: Pick<RegisteredFollowUpOptionsV37G3A, "projectRoot" | "dataRoot" | "workflowId">) {
	const data = v37G3ADataRootPath(options.projectRoot, options.dataRoot);
	...
	const admission = recomputeRegisteredRecoveryAdmissionV37G3A({ projectRoot: options.projectRoot, dataRoot: options.dataRoot, workflowId: options.workflowId, runRoot: resolve(options.projectRoot, binding.primary_run_root_location), confirmedAt: confirmation.confirmed_at, requestedAt: request.requested_at });
	if (admission.result !== "admitted" || !admission.opportunity) throw new Error("registered Recovery admission unavailable");
	return admission;
}

async function deriveBinding(options: RegisteredFollowUpOptionsV37G3A, candidate: RefinementCandidateV3) {
	...
	const loadedProfile = loadRegisteredFollowUpExecutionProfileV37G3A({ projectRoot: options.projectRoot, dataRoot: options.dataRoot, workflowId: options.workflowId });
...
export async function prepareRegisteredFollowUpV37G3A(options: RegisteredFollowUpOptionsV37G3A & { candidate: RefinementCandidateV3 }): Promise<RegisteredFollowUpBindingV37> {
...
function loadPrepared(options: RegisteredFollowUpOptionsV37G3A, historical = false) {
...
function runtimeTaskPolicy(profile: ReturnType<typeof loadRegisteredFollowUpExecutionProfileV37G3A>["profile"]) {
...
function runtimeBudgetProfile(profile: ReturnType<typeof loadRegisteredFollowUpExecutionProfileV37G3A>["profile"]): BoundedEditBudgetProfileV36 {
	return {
...
export function submitRegisteredFollowUpEvidenceV37G3A(options: RegisteredFollowUpOptionsV37G3A): { confirmation: FollowUpEvidenceConfirmationReceiptV37; request: FollowUpEvidenceSubmissionRequestV37 } {
	loadRegisteredFollowUpExecutionProfileV37G3A({ projectRoot: options.projectRoot, dataRoot: options.dataRoot, workflowId: options.workflowId });
	const root = workflowFollowUpRoot(options, false);
...
function inspectorFingerprint(): string {
	return digestObject({ inspector_id: INSPECTOR_ID, source_sha256: fileSha256(resolve(import.meta.dirname, "../inspect-v37g3a.ts")), registered_service_sha256: fileSha256(resolve(import.meta.dirname, "registered-follow-up-v37g3a.ts")) });
}

async function recompute(options: RegisteredFollowUpOptionsV37G3A, historical: boolean) {
	const prepared = loadPrepared(options, historical);
	const registered = loadWorkflowRegistrationV37G3A({ projectRoot: options.projectRoot, dataRoot: options.dataRoot, workflowId: options.workflowId, allowHistoricalReadOnly: historical ? true : undefined });
	const loadedProfile = loadRegisteredFollowUpExecutionProfileV37G3A({ projectRoot: options.projectRoot, dataRoot: options.dataRoot, workflowId: options.workflowId, ...(historical ? { allowHistoricalReadOnly: true as const } : {}) });
...
export async function admitRegisteredFollowUpV37G3A(options: RegisteredFollowUpOptionsV37G3A): Promise<RegisteredBoundFollowUpAdmissionV37> {
...
export async function recomputeRegisteredFollowUpAdmissionV37G3A(options: RegisteredFollowUpOptionsV37G3A & { historicalReadOnly?: true }): Promise<RegisteredBoundFollowUpAdmissionV37> {
...
export async function normalizeRegisteredBoundFollowUpV37G3A(options: RegisteredFollowUpOptionsV37G3A & { historicalReadOnly?: true }): Promise<CanonicalBoundStateAssessmentInputG2> {
	const admission = await recomputeRegisteredFollowUpAdmissionV37G3A(options);
	const root = workflowFollowUpRoot(options, false);
	const evidence = readJson<RegisteredBoundFollowUpEvidenceBodyV37>(root, "evidence.json", "follow-up Evidence");
	const binding = readJson<RegisteredFollowUpBindingV37>(root, "binding.json", "follow-up binding");
	return {
...
export function registeredFollowUpTreeDigestV37G3A(options: Pick<RegisteredFollowUpOptionsV37G3A, "projectRoot" | "dataRoot" | "workflowId">): string {
