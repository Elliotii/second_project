...
import { constructionAuthorityDigestsV37G3A, followUpAccessExpectationV37G3A, listRegisteredCasesFromHostRegistryV37G3A, loadRegisteredCaseFromHostRegistryV37G3A, primaryExecutionDeclarationV37G3A } from "./host-registry-v37g3a.ts";
...
export interface ProductCaseExecutionPortsV37G3A { primary:ProductPrimaryExecutionPortV37G3A; candidateProposal?:BoundedProposalPortV3; regressionValidation?:FauxValidationPortV3; followUpRuntime?:RegisteredFollowUpRuntimePortV37G3A; }
export interface ProductRealAccessAuthorizationV37G3A { case_id:string; manifest_body_digest:string; follow_up_execution_profile_digest:string; primary_access_expectation:ExecutionAccessExpectationV37G3A; follow_up_access_expectation:ExecutionAccessExpectationV37G3A; candidate_proposal_authority_digest:string; regression_authority_digest:string; }
...
const DEFAULT_CASE_PORTS:Readonly<Record<string,ProductCaseExecutionPortsV37G3A>>={
	[RECOVERY_CASE]:{primary:recoveryPrimaryPort,candidateProposal:deterministicCandidateProposalPort,regressionValidation:deterministicRegressionValidationPort,followUpRuntime:deterministicFauxFollowUpRuntimePortV37G3A},
...
export class ProductServiceV37G3A {
	...
	private readonly realAccessAuthorization:ProductRealAccessAuthorizationV37G3A|null;
	constructor(projectRoot:string,ports:ProductPortsV37G3A={}){
		...
		if(authorization&&stableJson(Object.keys(authorization).sort())!==stableJson(["candidate_proposal_authority_digest","case_id","follow_up_access_expectation","follow_up_execution_profile_digest","manifest_body_digest","primary_access_expectation","regression_authority_digest"].sort()))throw new Error("real-access Host construction authorization exact-key validation failed");
		...
		if(authorization&&authorized){if(!primaryExecutionDeclarationV37G3A(authorized.manifest).realAccessDeclared)throw new Error("real-access Host construction authorization does not match the Host-loaded Case profile");this.requireRealAuthority(authorized,authorization);}
		this.realAccessAuthorization=authorization?structuredClone(authorization):null;
	}
	private requireRealAuthority(loaded:LoadedRegisteredCaseV37G3A,authorization=this.realAccessAuthorization):void{const primary=primaryExecutionDeclarationV37G3A(loaded.manifest);if(!primary.realAccessDeclared)return;const follow=followUpAccessExpectationV37G3A(loaded.follow_up_execution_profile),derived=constructionAuthorityDigestsV37G3A(loaded.manifest),ports=this.executionPorts[loaded.manifest.case_id];if(!authorization||loaded.historical_read_only||authorization.case_id!==loaded.manifest.case_id||authorization.manifest_body_digest!==loaded.manifest.manifest_body_digest||authorization.follow_up_execution_profile_digest!==loaded.follow_up_execution_profile.follow_up_execution_profile_digest||stableJson(authorization.primary_access_expectation)!==stableJson(primary.accessExpectation)||stableJson(authorization.follow_up_access_expectation)!==stableJson(follow)||authorization.candidate_proposal_authority_digest!==derived.candidateProposalAuthorityDigest||authorization.regression_authority_digest!==derived.regressionAuthorityDigest)throw new Error("real-access Host construction authorization does not match the Host-loaded Case profile");if(!ports?.primary||!ports.candidateProposal||!ports.regressionValidation||!ports.followUpRuntime)throw new Error("real-access Case requires exact Primary, Candidate, Regression and follow-up Host execution ports");}
	private caseAvailable(loaded:LoadedRegisteredCaseV37G3A):boolean{const declaration=primaryExecutionDeclarationV37G3A(loaded.manifest),ports=this.executionPorts[loaded.manifest.case_id];if(loaded.historical_read_only||!ports)return false;if(!declaration.realAccessDeclared)return true;try{this.requireRealAuthority(loaded);return true;}catch{return false;}}
	...
	async act(workflowId:string,actionId:WorkflowActionIdV37G3A):Promise<WorkflowReadModelV37G3A>{
		...
		this.requireRealAuthority(registered.loadedCase);
	...
	private async executeAction(actionId:WorkflowActionIdV37G3A,caseId:string,workflowId:string):Promise<Array<{kind:string;id:string;digest:string}>>{
		...
		if(actionId==="produce_candidate"){
			if(!execution.candidateProposal)throw new Error("registered Case has no Host-constructed Candidate proposal port");const port=execution.candidateProposal;
		...
		if(actionId==="run_regression"){if(!execution.regressionValidation)throw new Error("registered Case has no Host-constructed Regression validation port");return this.runRegression(workflowId,execution.regressionValidation);}
	...
	private async runRegression(workflowId:string,configuredPort:FauxValidationPortV3){
		...
		const workspace=resolve(root,"workspace");mkdirSync(workspace,{recursive:true});writeFileSync(resolve(workspace,"subject.txt"),"broken\n");writeFileSync(resolve(workspace,"protected.txt"),"protected-stable\n");const verifier=this.publicationVerifier(root,workspace,"subject.txt","fixed\n"),regression=this.publicationVerifier(root,workspace,"protected.txt","protected-stable\n","protected");const validationRoot=registeredFollowUpPromotionValidationRootV37G3A(this.followOptions(workflowId));const rejectionPort:FauxValidationPortV3={execute:async({state:version})=>({settled:true,events:events(version.status,version.status==="accepted"?3:2)})};const realAccessDeclared=primaryExecutionDeclarationV37G3A(registered.loadedCase.manifest).realAccessDeclared;const port:FauxValidationPortV3=realAccessDeclared?configuredPort:(this.candidatePass?configuredPort:rejectionPort);const base=state.versions.find((entry)=>entry.state_digest===prior.state_digest)!;const manifest=registered.loadedCase.manifest;const validation=await executeSymmetricValidationV3({projectRoot:this.projectRoot,runRoot:validationRoot,validationId:`${workflowId}-validation`,projectId:registered.workflow.project_id,sourceWorkspaceRoot:workspace,task:verifier.task,verifier,regressions:[regression],baseState:base,candidateState:staged.state,providerModelProfileDigest:manifest.provider_profile_spec.spec_digest,toolProfileDigest:manifest.tool_profile_spec.spec_digest,budgetDigest:manifest.budget_profile_spec.spec_digest,hardConstraintsDigest:digestObject({command_profile_digest:manifest.command_profile_spec.spec_digest,stop_condition_profile_digest:manifest.stop_condition_profile_spec.spec_digest,regression_pack_digest:manifest.regression_pack_spec.spec_digest}),port});if(validation.validation.decision.result!=="promote")return [artifact("regression_rejected",validation.validation.validation_id,validation.validation)];const applied=await applyValidationDecisionV3({stateRoot,projectId:registered.workflow.project_id,runRoot:validationRoot,validationRef:validation.validationRef,stagedStateRoot:stagedRoot,candidateStateDigest:staged.state.state_digest,expectedActive:prior,immutableBasePrompt:SYSTEM_PROMPT,immutableBasePromptSha256:SYSTEM_PROMPT_SHA256});if(!applied.version)throw new Error("promoted State version is missing");return [artifact("promotion_decision",applied.decision.decision_id,applied.decision),artifact("state_version",String(applied.version.state_version),applied.version)];
	}
