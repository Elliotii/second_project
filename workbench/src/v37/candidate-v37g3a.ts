...
import { inspectRegisteredRecoveryAdmissionV37G3A } from "../inspect-v37g3a.ts";
...
export function validateFrozenPrimaryCandidateContentV37G3A(projectRoot: string, manifest: ReturnType<typeof loadWorkflowRegistrationV37G3A>["loadedCase"]["manifest"], candidateContent: string): void {
...
export async function producePromptCandidateV37G3A(options: {
	projectRoot: string;
	dataRoot: string;
	workflowId: string;
	runRoot: string;
	...
	const inspectedAdmission = inspectRegisteredRecoveryAdmissionV37G3A(options);
	...
	validateFrozenPrimaryCandidateContentV37G3A(options.projectRoot, manifest, edit.content);
