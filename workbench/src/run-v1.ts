...
export interface ExecuteV1RunCellOptions {
	...
	fakeScenario?: { initial: FakeAttemptModeV1B; child: FakeAttemptModeV1B; initialPatch?: string; childPatch?: string };
...
export async function executeV1RunCell(options: ExecuteV1RunCellOptions): Promise<ExecuteV1RunCellResult> {
	...
	const handle = createPublicPiRunCompositionV1B({ authority, factory: { create: (access) => createPiRunHandleV1({
		...
		pilotUsage: options.pilotUsage, realCallCounters: counters, runId: options.cell.planned_run_id, workspaceId,
		...(options.manifest.protocol_id === "v1c_budget_stop_correction" ? { budgetStopProtocol: "v1c_typed_predispatch_budget_stop_v1" as const } : {}),
		...(options.deterministicPausePhase ? { deterministicPausePhase: options.deterministicPausePhase } : {}),
		...(options.deterministicPauseRequestOrdinal !== undefined ? { deterministicPauseRequestOrdinal: options.deterministicPauseRequestOrdinal } : {}),
		...
		onProviderUsageCommitted: (event) => journal(options.runRoot, seq, "provider_usage_committed", event),
		onLocalBudgetStopRecorded: (signal) => journal(options.runRoot, seq, "local_budget_stop_recorded", signal),
		onLocalBudgetStopConsumed: (signal) => journal(options.runRoot, seq, "local_budget_stop_consumed", signal),
	}) } });
	const scenario: NonNullable<ExecuteV1RunCellOptions["fakeScenario"]> = options.fakeScenario ?? defaultScenario(options.cell);
	...
	try {
		...
		initial = await handle.runAttempt({ attemptId: initialAttemptId, prompt: instruction, invocation: options.cell.arm === "A" ? "prompt" : "skill", ...(stage1 ? { fakeMode: scenario.initial, fakePatch: scenario.initialPatch ?? patch } : {}) });
		...
		const runtimeDiagnostics: LocalBudgetStopSignalV1C[] = attempts.flatMap((attempt) => attempt.runtime_diagnostics ?? []);
		...
		const runResult: RunResultV1 = {
			...
			evidence: { task_digest: options.manifest.bindings.task_digests[task.task_id]!, workspace_source_digest: task.workspace_source_digest, prompt_digest: options.manifest.bindings.base_prompt_digest, skill_digest: options.cell.arm === "A" ? null : options.manifest.bindings.skill_digest, strategy_digest: options.manifest.bindings.strategy_digests[options.cell.strategy_id], tool_digest: options.manifest.bindings.tool_profile_digest, verifier_digest: verifierDigest(options.manifest, task.task_id), model_profile_digest: options.manifest.bindings.model_profile_digest, workbench_digest: options.manifest.workbench_source_digest, pi_digest: options.manifest.pi_commit, initial_payload_digest: initial.initial_dispatch.payload_sha256, skill_wrapper_bytes: options.cell.arm === "A" ? 0 : loaded.ref.wrapper_size_bytes, skill_body_bytes: options.cell.arm === "A" ? 0 : loaded.ref.source_size_bytes, attempts: attempts.map((attempt) => ({ attempt_id: attempt.attempt_id, ordinal: attempt.ordinal, parent_attempt_id: attempt.parent_attempt_id })), session_id: handle.sessionId, workspace_id: workspaceId, initial_verifier_status: initialVerifier.result.status, final_verifier_status: finalStatus, recovery_eligible: eligible, recovery_budget_available: eligible, recovery_started: recoveryStarted, provider_requests: usage.provider_requests, tool_calls: usage.tool_calls, tokens: usage.tokens, wall_time_ms: usage.active_execution_time_ms, cost_usd: usage.cost_usd, invalid_attribution: outcome.attribution, exclusion_preauthorized: outcome.excluded, terminal_refs: ["terminal.json"], ...(runtimeDiagnostics.length > 0 ? { runtime_diagnostics: runtimeDiagnostics } : {}) },
		};
