...
async function showSession(id) {
  ...
  const runs = text("div", "", "grid"); for (const run of session.runs) { const item = text("div", "", "run"); item.append(text("h3", run.run_id), metric("Settled", run.settled), metric("Provider requests", run.provider_requests), metric("Tool calls", run.tool_call_count), metric("Context reconstructed", run.context_reconstructed), metric("Source ref", run.source_ref)); runs.append(item); } root.append(card("Runs", runs));
}
