...
const application = new WorkbenchApplicationV36G1({ legacy, controlPlane: new InteractiveControlPlaneV36({ dataRoot: resolve(runtimeRoot, "v36-data"), registry }) });
const loopback = createWorkbenchLoopbackServerV36G1(application);
const address = await loopback.start(options.port);
process.stdout.write(`${JSON.stringify({ schema_version: 1, event: "v36g1_demo_started", host: address.host, port: address.port, url: address.url, mode: "deterministic_faux_zero_access", project_commands: 0, docker_project_command_executions: 0 })}\n`);
if (options.smoke) {
	await loopback.stop();
	process.stdout.write(`${JSON.stringify({ schema_version: 1, event: "v36g1_demo_stopped", listener_count: 0 })}\n`);
}
