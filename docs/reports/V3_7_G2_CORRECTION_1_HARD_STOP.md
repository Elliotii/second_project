...
```yaml
status: RESOLVED_BY_AUTHORITY_RECALIBRATION
disposition: RESUME_CORRECTION_1_WITHOUT_REGISTERED_PROFILE_EXPANSION
correction_round: 1
...
resolution: original_coverage_categories_do_not_require_a_registered_end_to_end_cross_product
```
...
After rechecking the accepted authority in order, Main determined that the recommendation
above was too broad. Charter Section 6.9 and Prompt Section 8 list negative
Verifier/Outcome behavior, both normalizers and reassessment/rollback as coverage
categories; they do not require every category to be crossed into a second registered
V3.7 execution scenario. The accepted Amendment freezes one profile, while Charter
Section 7.4 freezes the full deterministic follow-up route as PASS-to-retain and permits
non-registered tampered material for negative coverage.

The user approved the recalibrated route. Correction 1 resumes with the single profile
unchanged. It must remove source-family branching from canonical decision logic, exercise
the real negative Verifier/Inspector boundaries, preserve accepted legacy negative
reassessment/rollback coverage and report that no separate registered V3.7 negative
end-to-end Run was created. Goal 2 remains unaccepted and Goal 3 remains locked.
