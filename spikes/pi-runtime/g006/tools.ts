...
export type G006ToolContext = {
	workspace: string;
	record: (record: ToolAuditRecord) => void;
};

function exactWorkspacePath(context: G006ToolContext, relativePath: (typeof READ_FILES)[number]): string {
	return resolve(context.workspace, relativePath);
}

export function createG006Tools(): Array<AgentHarnessTool<G006ToolContext>> {
	const readTool: AgentHarnessTool<G006ToolContext, typeof NO_PARAMETERS, { files: string[]; bytes: number }> = {
	...
	const writeTool: AgentHarnessTool<G006ToolContext, typeof WRITE_PARAMETERS, ToolAuditRecord> = {
		name: "write_source",
		label: "Write source",
	...
	const testTool: AgentHarnessTool<G006ToolContext, typeof NO_PARAMETERS, Record<string, unknown>> = {
...
export const G006_TOOL_NAMES = ["read_task_and_source", "write_source", "run_public_tests"] as const;
